...
class DensePoseMaskedColormapResultsVisualizer(DensePoseResultsVisualizer):
    ...
    def visualize_iuv_arr(self, context, iuv_arr: np.ndarray, bbox_xywh) -> None:
        ...
        # Convert the mask to a visible format and save
        visible_mask = mask * 255  # Convert binary mask to grayscale
        # cv2.imwrite("segmentation_mask.jpg", visible_mask)

        # Apply the mask and visualize as needed
        image_bgr = self.mask_visualizer.visualize(image_bgr, mask, matrix, bbox_xywh)

        # # Optionally save the final visualized image
        cv2.imwrite("final_visualized_image.jpg", image_bgr)
