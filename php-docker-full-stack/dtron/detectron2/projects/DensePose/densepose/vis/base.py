...
class MatrixVisualizer:
    ...
    def visualize(self, image_bgr, mask, matrix, bbox_xywh):
        ...
        # Create a blank (black) image with the same dimensions as the input image
        image_target_bgr = np.zeros_like(image_bgr)
        ...
        # Scale matrix values for color mapping
        ...
        # Apply color map to the scaled matrix
        matrix_vis = cv2.applyColorMap(matrix_scaled_8u, self.cmap)

        # Use the mask to combine only the DensePose results where the mask is non-zero
        mask_fg = np.tile(mask[:, :, np.newaxis], [1, 1, 3])  # Convert to 3-channel mask
        image_target_bgr[y : y + h, x : x + w, :] = np.where(mask_fg, matrix_vis, 0)
